'''
library for simulation of quantum annealing
...
class QMC :
    ...
    def move(self,conf):
        ...
        delta_E = delta_costc = delta_costq_1 = delta_costq_2 = delta_costq_3 = delta_costq_4 = 0
        ...
        for j in range(self.NCITY):
            ...
            delta_Ec += 2*(-l_p_j*conf[c][a][p] - l_q_j*conf[c][a][q])*(conf[c][a-1][j]+conf[c][(a+1)%self.TOTAL_TIME][j]) \
                           +2*(-l_p_j*conf[c][b][p] - l_q_j*conf[c][b][q])*(conf[c][b-1][j]+conf[c][(b+1)%self.TOTAL_TIME][j])

        para = (1/self.BETA)*\
            math.log(math.cosh(self.BETA*self.ANN_PARA/self.TROTTER_DIM)/math.sinh(self.BETA*self.ANN_PARA/self.TROTTER_DIM))
        ...
        delta_E = delta_costc/self.TROTTER_DIM+para*(delta_costq_1+delta_costq_2+delta_costq_3+delta_costq_4)

        # accept spin flipping by min(1,exp(-beta*delta_cost))
        if delta_cost <= 0:
            conf[c][a][p]*=-1
            conf[c][a][q]*=-1
            conf[c][b][p]*=-1
            conf[c][b][q]*=-1
        elif np.random.random() < np.exp(-self.BETA*delta_E):
            conf[c][a][p]*=-1
            conf[c][a][q]*=-1
            conf[c][b][p]*=-1
            conf[c][b][q]*=-1
